...
API de produtividade pessoal (tarefas, hábitos, autenticação) em **FastAPI + Postgres**, com pipeline de CI/CD, infraestrutura como código no Azure, mensageria assíncrona e observabilidade. Foi construída como material de portfólio para uma posição de **Cloud/DevOps Jr**.
...
Todos os segredos (`db_password`, `rabbitmq_url`, `google_client_id`) são `variable` `sensitive = true`, passados via `terraform.tfvars` local. Esse arquivo, junto com `*.tfstate` e `.terraform/`, está no `.gitignore` e nunca foi commitado.
...
- **JWT**: `HS256`, expiração de 7 dias (`ACCESS_TOKEN_EXPIRE_MINUTES = 60*24*7`), assinado com `SECRET_KEY` (variável de ambiente que tem um valor padrão de desenvolvimento no código, deve ser sobrescrita em produção; ver seção 3 sobre o Terraform não declarar essa variável).
